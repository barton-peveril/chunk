...
RUN apt-get update ; apt-get install -y moosefs-chunkserver nano sed ; rm -rf /var/lib/apt/lists/* && mv /etc/mfs/mfschunkserver.cfg.sample /etc/mfs/mfschunkserver.cfg ; mv /etc/mfs/mfshdd.cfg.sample /etc/mfs/mfshdd.cfg ; echo "/mnt/chunks/ -5GiB" >> /etc/mfs/mfshdd.cfg ; sed 's/# LABELS =/LABELS = D/g' /etc/mfs/mfschunkserver.cfg ;
